...
export function useChildren(options: UseChildrenOptions = {}): UseChildrenReturn {
 ...
 // ===============================================
// AUXILIARES PARA CREATE CHILD
// ===============================================

const buildInsertData = (childData: ChildInsert, userId: string) => ({
...
const handleInsertError = (insertError: any) => {
...
const createParentRelation = async (userId: string, newChildId: string, supabase: any) => {
  try {
...
const auditCreateChild = async (newChild: Child) => {
  try {
    await auditSensitiveAccess(
      'CREATE_CHILD',
      newChild.id,
...
// ===============================================
// FUNCIÓN PRINCIPAL CREATE CHILD
// ===============================================
...
const createChild = useCallback(async (childData: ChildInsert): Promise<Child> => {
  ...
  try {
    ...
    // Preparar datos
    const insertData = buildInsertData(childData, userId);
    ...
    if (insertError) handleInsertError(insertError);
    ...
    await createParentRelation(userId, newChild.id, supabase);

    // Refrescar lista
    await fetchChildren();

    // Auditoría
    await auditCreateChild(newChild);
